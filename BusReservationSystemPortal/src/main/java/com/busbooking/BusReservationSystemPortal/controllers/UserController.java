...
public class UserController {
    ...
    public ResponseEntity<User> updateUser(@Valid @RequestBody User user, @RequestParam(required = false) String key) throws UserException {
        User udpateuser = userService.updateUser(user, key);
    ...
    public ResponseEntity<User> viewUser(@PathVariable("userId") Integer userId, @RequestParam(required = false) String key) throws AdminException, UserException {
        User viewuser = userService.viewUserById(userId, key);
    ...
    public ResponseEntity<List<User>> viewUsers(@RequestParam(required = false) String key) throws AdminException, UserException {
        List<User> users = userService.viewUsers(key);
